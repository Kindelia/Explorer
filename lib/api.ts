...
const fetchApi = async <T>(endpoint: string, init?: RequestInit) => {
  const response = (await fetch(
    `${process.env.API_URL || 'http://localhost:8000'}${endpoint}`,
    init
  ).then((res) => res.json())) as Response<T>
...
export async function get_block(id: bigint | T.Hash): Promise<T.BlockJson> {
  return fetchApi(`/blocks/${id}`)
}

export async function get_block_content(id: bigint | string) {
...
export const get_function_state = (id: bigint | string) =>
  fetchApi<T.StatementJson>(`/functions/${id}/state`)
